...
// ____________________________________________________________
//
const fillTextbox = (ui: ReturnType<typeof within>, { name, value }: { name: string; value: string }) => {
...
// ____________________________________________________________
const fillAndSubmit = (inlinePlay?: (ctx: any) => Promise<void>) => async (ctx: any) => {
...
export const SucceedSubmit: Story = {
  ...
  parameters: {
    msw: {
      handlers: [rest.post('http://api.example.com', (req, res, ctx) => res(ctx.status(201), ctx.json(req.body)))]
    }
  }
}
